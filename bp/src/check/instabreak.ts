...
// Predefined sets for fast lookup
...
const INSTA_BREAKABLE_SET = new Set([
    ...
    "minecraft:sandstone", "minecraft:quartz_block", "minecraft:deepslate", "minecraft:dirt",
    "minecraft:grass_block", "minecraft:sand", "minecraft:obsidian", "minecraft:crying_obsidian",
    "minecraft:oak_log", "minecraft:birch_log", "minecraft:spruce_log", "minecraft:jungle_log",
    "minecraft:acacia_log", "minecraft:dark_oak_log", "minecraft:oak_wood", "minecraft:birch_wood",
    "minecraft:spruce_wood", "minecraft:jungle_wood", "minecraft:acacia_wood", "minecraft:dark_oak_wood",
    "minecraft:anvil", "minecraft:diamond_block", "minecraft:gold_block", "minecraft:iron_block",
    ...
    "minecraft:diamond_ore", "minecraft:copper_ore", "minecraft:emerald_ore", "minecraft:quartz_ore",
    "minecraft:deepslate_coal_ore", "minecraft:deepslate_iron_ore", "minecraft:deepslate_gold_ore",
    "minecraft:deepslate_redstone_ore", "minecraft:deepslate_diamond_ore", "minecraft:deepslate_copper_ore",
    "minecraft:deepslate_emerald_ore", "minecraft:deepslate_lapis_ore", "minecraft:lapis_ore",
    "minecraft:ancient_debris",
]);

// Helper function
function isTool(itemStack?: ItemStack) {
    return itemStack ? TOOL_SET.has(itemStack.type.id) : false;
}

// Event handler
function onBlockBreak({ player, brokenBlockPermutation, itemStackBeforeBreak: tool, block }: PlayerBreakBlockAfterEvent) {
    if (player.isOp() || ["minecraft:air", "minecraft:netherrack"].includes(brokenBlockPermutation.type.id)) return;

    player.breakData ??= { brokenBlocks: [], brokenAmount: 0 } as InstabreakData;

    player.breakData.brokenBlocks.push({ blockPermutation: brokenBlockPermutation, blockPosition: block.location });

    const usingTool = isTool(tool);
    const hasHaste = player.getEffect("minecraft:haste");

    const effLevel = tool?.getComponent("enchantable")?.getEnchantment("minecraft:efficiency")?.level ?? 0;
    const instabreakable = INSTA_BREAKABLE_SET.has(brokenBlockPermutation.type.id);

    if (!(hasHaste && usingTool) || (usingTool && effLevel >= 2 && instabreakable)) {
...
function tickEvent(player: Player) {
    const data = player.breakData;
    if (!data || data.brokenBlocks.length === 0) return;

    if (data.brokenAmount > MAX_BREAK_IN_TICK) {
        system.runJob(recoverBlocks(data.brokenBlocks, player.dimension));
        player.flag("Insteabreak", "A", "World", { breakAmount: data.brokenAmount });
    }

    // Reset break data
    player.breakData = { brokenBlocks: [], brokenAmount: 0 };
}

// Generator to restore blocks
function* recoverBlocks(blocks: BrokenBlockList, dimension: Dimension) {
    for (const { blockPermutation, blockPosition } of blocks) {
        ...
        dimension.getEntities({ location: blockPosition, type: "minecraft:item", maxDistance: 2, minDistance: 0 })
